...
const ChatPanel = ({
  ...
  useEffect(() => {
    ...
    recognitionRef.current.onresult = (event: any) => { setValue((prev) => prev + " " + event.results[0][0].transcript); adjustHeight(); };
  ...
  useEffect(() => {
    if (initialPrompt && initialPrompt.trim() !== "" && !hasAutoTriggered) {
      ...
      startTransition(() => { handleSendMessageWithPrompt(initialPrompt); });
    }
  }, [initialPrompt]);
  ...
  const handleSendMessageWithPrompt = async (promptText: string) => {
    ...
    if (isModifyMode) {
      ...
      if (storedContainers) { try { const c = JSON.parse(storedContainers); if (c[projectId]) currentContainerId = c[projectId].containerId; } catch(e) {} }
      if (!currentContainerId) { const iframe = document.querySelector('iframe'); if (iframe && iframe.src) { const m = iframe.src.match(/\/preview\/(\d+)\//); if (m) currentContainerId = m[1]; } }
    }
    ...
    if (!canProceed) {
      ...
      setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: !isAuthenticated ? "You've run out of guest credits. Please sign in to continue building!" : "You've run out of credits for today. Upgrade your plan to continue building!" }]);
      return;
    }
    ...
    if (selectedElementId && isModifyMode) fullPrompt = `[TARGET: ${selectedElementId}]\n${fullPrompt}`;
    try {
      const API_URL = "https://api.mycodex.dev";
      const endpoint = isModifyMode ? `${API_URL}/modify` : `${API_URL}/build`;
      const requestBody = { prompt: fullPrompt, stack: selectedStack, ...(isModifyMode && { projectId, containerId: currentContainerId }), ...(uploadedImages.length > 0 && { images: uploadedImages }) };
      ...
      if (data.success && data.url) {
        ...
        setMessages((prev) => [...prev, { id: (Date.now() + 2).toString(), role: "assistant", content: `🚀 Your app is ${isModifyMode ? 'modified' : 'ready'}!\n\nURL: ${previewUrl}` }]);
        markAsReady(previewUrl);
      } else { throw new Error(data.error || "Failed to generate application"); }
    } catch (error: any) {
      setMessages((prev) => [...prev, { id: (Date.now() + 2).toString(), role: "assistant", content: `⚠️ Backend Error: ${error.message || "Failed to connect to the generator API."}` }]);
      onHealthCheckStatus?.(false);
    } finally { setIsTyping(false); setIsModifying(false); }
  };

  const markAsReady = async (previewUrl: string) => {
    try { await fetch(previewUrl, { method: "HEAD", mode: "no-cors" }); } catch(e) {}
  ...
  const triggerBuild = async (prompt: string, selectedStack: "react" | "nextjs" | "vue" | "html") => {
    ...
    if (isAuthenticated) { canProceed = isUnlimited || await authDeductCredit(); } else { canProceed = guestDeductCredit(); }
    if (!canProceed) {
      setShowUpgradePrompt(!isAuthenticated);
      setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: "You've run out of credits for today. Sign in or upgrade to continue building!" }]);
      return;
    }
    setMessages((prev) => [...prev, { id: Date.now().toString(), role: "user", content: prompt }]);
    setIsTyping(true); setValue(""); setAttachments([]); adjustHeight(true);
    try {
      ...
      if (result.success && result.data) {
        ...
        if (data.success && data.url) {
          const previewUrl = normalizePreviewUrl(data.url);
          onGeneratedUrl?.(previewUrl);
          setMessages((prev) => [...prev, { id: (Date.now() + 2).toString(), role: "assistant", content: `🚀 Your app is ready!\n\nURL: ${previewUrl}` }]);
          markAsReady(previewUrl);
        } else {
          setMessages((prev) => [...prev, { id: (Date.now() + 2).toString(), role: "assistant", content: data.response || data.message || "I'll make those changes for you. Updating the application now..." }]);
        }
      } else {
        const errorMessage = result.error === 'timeout' ? "The request timed out after 10 minutes. Please try again." : `⚠️ Backend Error: ${result.error || "Please check your connection and try again."}`;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (showCommandPalette) {
      ...
      else if (e.key === "Tab" || e.key === "Enter") { e.preventDefault(); if (activeSuggestion >= 0) { setValue(commandSuggestions[activeSuggestion].prefix + " "); setShowCommandPalette(false); } }
  ...
  const handleSendMessage = async () => {
    ...
    setIsTyping(true); setIsModifying(isModifyMode); setValue(""); setAttachments([]); adjustHeight(true);
    onClearElement?.();
    let fullPrompt = value;
    let uploadedImages: {url: string}[] = [];
    if (attachments.length > 0) { uploadedImages = attachments.map(url => ({ url })); fullPrompt += `\n\n[USER INSTRUCTION] User uploaded ${attachments.length} image(s). Please incorporate the design elements seen in these images exactly as requested in the main prompt.`; }
    if (selectedElementId && isModifyMode) fullPrompt = `[TARGET: ${selectedElementId}]\n${fullPrompt}`;
    try {
      const API_URL = "https://api.mycodex.dev";
      const endpoint = isModifyMode ? `${API_URL}/modify` : `${API_URL}/build`;
      let currentContainerId = null;
      if (isModifyMode) { const sc = localStorage.getItem('active_containers'); if (sc) { try { const c = JSON.parse(sc); if (c[projectId]) currentContainerId = c[projectId].containerId; } catch(e) {} } }
      const requestBody = { prompt: fullPrompt, stack: selectedStack, ...(isModifyMode && { projectId, containerId: currentContainerId }), ...(uploadedImages.length > 0 && { images: uploadedImages }) };
      const response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(requestBody) });
      if (!response.ok) throw new Error(`Server returned ${response.status}`);
      const data = await response.json();
    ...
    <div className="flex flex-col h-full bg-background/50 backdrop-blur-xl relative">
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {messages.map((message) => (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={message.id}
            ...
            <div className={`max-w-[85%] rounded-2xl p-4 ${message.role === "user" ? "bg-primary text-primary-foreground ml-4" : "bg-muted/50 border border-white/5 mr-4"} shadow-lg backdrop-blur-sm`}>
      ...
      {showUpgradePrompt && !isAuthenticated && (
        <div className="px-4 py-2 bg-primary/10 border-t border-primary/20 flex items-center justify-between">
          ...
          <button onClick={() => navigate('/login')} className="text-xs bg-primary text-primary-foreground px-3 py-1.5 rounded-md hover:bg-primary/90 transition-colors">Sign In</button>
        </div>
      )}
      {showUpgradePrompt && isAuthenticated && (
        <div className="px-4 py-2 bg-primary/10 border-t border-primary/20 flex items-center justify-between">
          <p className="text-sm text-primary">Upgrade your plan to get more daily credits!</p>
          <button onClick={() => navigate('/pricing')} className="text-xs bg-primary text-primary-foreground px-3 py-1.5 rounded-md hover:bg-primary/90 transition-colors">Upgrade Plan</button>
        </div>
      )}
      ...
      <div className="p-4 border-t border-white/5 bg-background/80 backdrop-blur-xl shrink-0">
            ...
            <AnimatePresence>
              {attachments.map((url, i) => (
                <motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="relative group">
                  ...
                  <button onClick={() => removeAttachment(i)} className="absolute -top-2 -right-2 p-1 bg-destructive text-destructive-foreground rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
                    <X className="w-3 h-3" />
                  </button>
                </motion.div>
              ))}
        ...
        <div className="relative flex items-end gap-2 bg-muted/30 border border-white/10 rounded-2xl p-2 focus-within:border-primary/50 transition-colors shadow-inner">
          {showCommandPalette && (
            <div ref={commandPaletteRef} className="absolute bottom-full left-0 w-64 mb-2 bg-background/95 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden shadow-2xl z-50 transform origin-bottom">
              ...
              <div className="p-1">
                {commandSuggestions.map((suggestion, index) => (
                  <button key={suggestion.prefix} onClick={() => { setValue(suggestion.prefix + " "); setShowCommandPalette(false); textareaRef.current?.focus(); }}
                    className={`w-full text-left px-3 py-2.5 rounded-lg flex items-center gap-3 transition-all ${index === activeSuggestion ? "bg-primary text-primary-foreground shadow-sm" : "hover:bg-muted text-foreground"}`}>
          ...
          <div className="flex gap-1 pb-1 px-1">
            ...
            <button onClick={() => fileInputRef.current?.click()} disabled={isUploading || attachments.length >= 3}
              className={`p-2 text-muted-foreground hover:text-foreground rounded-xl transition-colors ${isUploading ? "opacity-50 cursor-not-allowed" : "hover:bg-muted/50"}`} title="Attach image (max 3)">
              <FileUploadZone onFileSelect={(file) => {
                const dt = new DataTransfer(); dt.items.add(file);
                if (fileInputRef.current) { fileInputRef.current.files = dt.files; handleFileUpload({ target: fileInputRef.current } as any); }
              }}>
                {isUploading ? <RefreshCw className="w-5 h-5 animate-spin" /> : <Paperclip className="w-5 h-5" />}
              </FileUploadZone>
            </button>
            <button onClick={toggleVoiceRecording}
              className={`p-2 rounded-xl transition-all shadow-sm ${isRecording ? "bg-destructive text-destructive-foreground animate-pulse shadow-destructive/20" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"}`} title="Voice input">
              <Mic className="w-5 h-5" />
            </button>
          </div>

          <textarea ref={textareaRef} value={value} onChange={(e) => { setValue(e.target.value); adjustHeight(); }} onKeyDown={handleKeyDown}
            placeholder={!hasCredits ? "Out of credits" : isModifying && selectedElementId ? "Modifying target element..." : isModifying ? "Modify project..." : "Describe your app..."}
            className="flex-1 bg-transparent border-none resize-none focus:ring-0 py-3 px-2 text-sm max-h-[120px] scrollbar-thin scrollbar-thumb-white/10" rows={1} disabled={!hasCredits} />

          <button onClick={handleSendMessage} disabled={!value.trim() || isPending || !hasCredits}
